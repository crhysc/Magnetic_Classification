...
file_object = open('neal_data.json','r')
neal_dict = json.load(file_object)
j = len(neal_dict)



rf = open('left_over_neal_data_reformatted.txt' ,"r")
lines = rf.readlines()
rf.close()
compounds =  [] 
lineNum = []
for iline in range(len(lines)):
    ...
    if iline + 1 != len(lines)  and lines[iline+1][0] == "-" and lines[iline][0] != '-':
        ...
        neal_dict.update({'id' + str(j) : {'composition': compound, 
                                                'neal_temperature': neal_T, 
                                                'neal_temperature_uncertaintity':  neal_uncertain,
        ...
        j+=1
            ...
            j+=1
with open('neal_data_afterLeftOver.json', 'w') as fp:
