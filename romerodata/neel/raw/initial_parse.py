...
rf = open('NeelTemperature_data.txt' ,"r")
lines = rf.readlines()
rf.close()
no_comp_index = []
no_comp = []
...
for iline in range(len(lines)):
    ...
    if iline + 1 != len(lines) and '-' not in  lines[iline].split()[0] and '&' not in  lines[iline].split()[0] and '#' not in  lines[iline].split()[0] and lines[iline+1][0] != "-"  and lines[iline+1][0] != "&"  :
        ...
        curie_dict.update({'id' + str(j) : {'composition': compound, 
                                            'neal_temperature': curie_T, 
                                            'neal_temperature_uncertaintity':  curie_uncertain,
    ...
    if 'x' in lines[iline].split()[0]:
        ...
        while char == '&':
            ...
            curie_dict.update({'id' + str(j) : {'composition' : compound.replace("x" , x), 
                                                        'neal_temperature': curie_T, 
                                                        'neal_temperature_uncertaintity':  curie_T_uncertainty,
                                                        'reference_numbers': reference_numbers,
                                                        'line_in_neal_file' : str(iline + i + 1),
                                                        'additional_info':additional_info
                                                        }})
...
with open('neal_data.json', 'w') as fp:
...
with open('left_over_neal_data.txt', 'a') as the_file:
    for line in lines2:
        the_file.write(line)
